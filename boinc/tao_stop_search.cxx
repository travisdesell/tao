...
/**
 *  Steps:
 *      1. get all searches from the database.
 *      2. get all workunit information for those searches.
 *
 *      3. for each non-terminated search
 *          a. generate parameters / seed
 *          b. append workunit information (credit, fpops)
...
void main_loop() {
    ...
    while (1) {
        ...
        if (n > CUSHION) {
            ...
            if (retval) {
                log_messages.printf(MSG_CRITICAL, "make_jobs failed with error: %s\n", boincerror(retval) );
                exit(retval);
            }
...
int main(int argc, char** argv) {
    ...
    for (i=1; i<argc; i++) {
        if (is_arg(argv[i], "d")) {
            ...
            printf("%s\n", SVN_VERSION);
            exit(0);
        } else if (is_arg(arg[i], "c") || is_arg(argv[i], "create_table")) {
            create_table = true;
        } else {
    ...
    if (create_table) {
        create_table();
        log_messages.printf(MSG_CRITICAL, "Successfully created workunit_information database table.\n");
        exit(0);
    }
