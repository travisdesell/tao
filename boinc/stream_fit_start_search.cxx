...
//		double integral_flops = 0;
		for (i = 0; i < ap->number_integrals; i++) {
			calc_prob_count += (ap->integral[i]->r_steps / 100.0) * (ap->integral[i]->mu_steps / 100.0) * (ap->integral[i]->nu_steps / 100.0);
//			integral_flops += ap->integral[i]->mu_steps * ap->integral[i]->r_steps * (5.0 + ap->integral[i]->nu_steps * (7.0 + 5.0 * ap->number_streams + ap->convolve * (35.0 + 52.0 * ap->number_streams)));
		...
		printf("integral_flops: %lf, likelihood_flops: %lf", integral_flops, likelihood_flops);
